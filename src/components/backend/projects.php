...
where SiteRequisitionMaster.Cancelied!=1 and TLReject!=1 and SLReject!=1 and FLReject!=1 
and UserSRNJobPermission.UserID='" . $userlevel . "'";
if($job_level != NULL){
     if($job_level == '1st Level'){
           $sql=$sql." AND UserDocumentAuthorization.FirstLevel = '" . $userlevel . "'";
    }elseif($job_level == '2nd Level'){
           $sql=$sql." AND UserDocumentAuthorization.SecondLevel = '" . $userlevel . "'";
    }elseif($job_level == '3rd Level'){
           $sql=$sql." AND UserDocumentAuthorization.ThirdLevel = '" . $userlevel . "'";
